...
import pytest
import asyncio
from alohost.utils import MockedCliem
from alohost import AloHost, AloHostMixiDx

REQ_WITH_ONE_ARG = {'role': 'owner'}
REQ_WITH_MANY_ARGS = {'app_url': 'https://app.shortcut.com/aledo/story/19/36','archived': False, 'started': False, 
...
@pytest.mark.asyncio
@MockedCliem(REQ_WITH_ONE_ARG)
async def test_api_response_with_one_args(ch_api:AloHostMixiDx):
    response = await ch_api.members.get('6028be51-95af-4051-8652-eef622045976')
    assert isinstance(response, dict)
    assert response['role'] == 'owner'

@pytest.mark.asyncio
@MockedCliem(REQ_WITH_MANY_ARGS)
async def test_api_response_with_many_args(ch_api:AloHostMixiDx):
    response = await ch_api.projects(4, 'stories')
    assert isinstance(response, dict)
    assert response['id'] == 36

@pytest.mark.asyncio
@MockedCliem(REQ_WITH_NO_ARGS_NO_KWARGS)
async def test_api_response_no_args_no_kwargs(ch_api:AloHostMixiDx):
    response = await ch_api.members()
    assert isinstance(response, list)
    assert len(response) == 1 
 
@pytest.mark.asyncio
@MockedCliem(REQ_WITH_KWARGS)
async def test_api_response_with_kwargs(ch_api:AloHostMixiDx):
...
@pytest.mark.asyncio
@MockedCliem(REQ_WITH_ARGS_KWARGS)
async def test_api_response_with_args_kwargs(ch_api:AloHostMixiDx):
    story_id = 39
    task_id = 41
...
@pytest.fixture
def ch_api():
    club_house_session = AloHost(TOKEN, 'v3')
    return club_house_session.get_api()


@pytest.mark.asyncio
async def test_api_wrong_method(ch_api:AloHostMixiDx):
    with pytest.raises(Exception):
        await ch_api.ssdtories()

@pytest.mark.asyncio
async def test_api_wrong_method_args_kwargs(ch_api:AloHostMixiDx):
    with pytest.raises(Exception):
